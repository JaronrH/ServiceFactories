...
{
    public class TransientLifetimeTests : ServiceCollectionTestsBase
    {
        public TransientLifetimeTests() : base(s => AddTestComponents(s, ServiceLifetime.Transient)) { }
                ...
                foreach (var serviceType in Enum.GetValues(typeof(ServiceType)).Cast<ServiceType>())
                {
                    if (serviceType == ServiceType.Custom)
                        if (lifetime == ServiceLifetime.Transient)
                            Assert.NotEqual(factory1.GetAccessor(ServiceKey(lifetime, serviceType)), factory2.GetAccessor(ServiceKey(lifetime, serviceType)));
                        else
                            Assert.Equal(factory1.GetAccessor(ServiceKey(lifetime, serviceType)), factory2.GetAccessor(ServiceKey(lifetime, serviceType)));
                    else
                        foreach (var resolverLifetime in new[] { ServiceLifetime.Singleton, ServiceLifetime.Transient })
                            if (lifetime == ServiceLifetime.Transient)
                                Assert.NotEqual(factory1.GetAccessor(ServiceKey(lifetime, serviceType, resolverLifetime)), factory2.GetAccessor(ServiceKey(lifetime, serviceType, resolverLifetime)));
                            else
        ...
        public async Task ResolverLifetimeChecksAsync()
        {
            ...
            foreach (var lifetime in Enum.GetValues(typeof(ServiceLifetime)).Cast<ServiceLifetime>())
            {
                foreach (var serviceType in Enum.GetValues(typeof(ServiceType)).Cast<ServiceType>())
                {
                    ...
                    {
                        if (lifetime == ServiceLifetime.Transient)
                            Assert.NotEqual(await factory1.ResolveAsync(ServiceKey(lifetime, serviceType, ServiceLifetime.Singleton), "Test"), await factory2.ResolveAsync(ServiceKey(lifetime, serviceType, ServiceLifetime.Singleton), "Test"));
                        else
        ...
        public void ResolverLifetimeChecksSync()
        {
            ...
            foreach (var lifetime in Enum.GetValues(typeof(ServiceLifetime)).Cast<ServiceLifetime>())
            {
                foreach (var serviceType in Enum.GetValues(typeof(ServiceType)).Cast<ServiceType>())
                {
                    ...
                    {
                        if (lifetime == ServiceLifetime.Transient)
                            Assert.NotEqual(factory1.Resolve(ServiceKey(lifetime, serviceType, ServiceLifetime.Singleton), "Test"), factory2.Resolve(ServiceKey(lifetime, serviceType, ServiceLifetime.Singleton), "Test"));
                        else
